...
def get_blog_list_common_data(request,blogs_all_list):
    #添加页码
    paginator =Paginator(blogs_all_list,settings.EACH_PAGE_BLOGS_NUMBER)   #每6个文章分一次页
    page_num = request.GET.get('page', 1)   #获取URL的页面参数
    page_of_blogs = paginator.get_page(page_num)
    current_page_num = page_of_blogs.number   #获取当前页面并显示其前后两页的内容
    page_range = list(range(max(current_page_num - 2,1),current_page_num))+\
                 list(range(current_page_num,min(current_page_num + 2,paginator.num_pages)+1))
    if page_range[0]-1>=2:    #添加首尾页和省略号
        page_range.insert(0,'...')
